...
aggregated_frequencies = []  # List to store final total values
...
if num_bins <= bin_A:
    ...
    aggregated_frequencies = list(bin_frequencies)

    aggregated_bins = [f"{interval.left /1e6:.1f}M - {interval.right /1e6:.1f}M" for interval in bin_totals['bin']]

    bar_colors = ['#a6c8ff'] * len(aggregated_frequencies) # light blue
else:
    aggregated_frequencies = list(bin_frequencies[:bin_A])
    ...
    for bin_limit, group_size, color in aggregation_rules:
        while i < min(bin_limit, num_bins):
            ...
            aggregated_frequencies.append(bin_frequencies[i:end_idx].sum())
...
print('total: ', aggregated_frequencies[:5])

# Plot total amount per bin
plt.figure(figsize=(12, 7))
plt.bar(range(len(aggregated_frequencies)), aggregated_frequencies, width=0.8, align='center', alpha=0.7, color=bar_colors, edgecolor='black', log=True)

# Set x-ticks with bin ranges

plt.xticks(range(len(aggregated_frequencies)), aggregated_bins ,fontsize=7, rotation=45)

# Labels and title
#plt.xlabel("Intervalos de Vlr_Empenhado (em milhões de Reais - R$)")
#plt.ylabel("Frequência total")
#plt.title("Frequência por Intervalo")
