...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
    ...
    model = DEC(cluster_number=num_clusters, hidden_dimension=num_clusters, encoder=autoencoder.encoder)
    if cuda:
        model.cuda()
    ...
    # X são as features processadas pelo DEC
    # são todas as probabilidades de um exemplo estar associado a um cluster
    # X tem tamanho (len(ds_train), num_clusters)
    
    # predicted retorna o index da maior probabilidade (no caso index = cluster)
    # predicted é uma lista de tamanho (ds_train)
    
    # Save 'predicted' to a file
